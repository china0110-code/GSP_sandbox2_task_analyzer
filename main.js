...
/* ══════════════════════════════════════════════════════════
   PDF エクスポート — html2canvas 方式
   Web 画面の HTML をそのままキャプチャして PDF に焼き付ける。
   p.1 : 4象限マップ ＋ 象限説明カード（チャートエリア全体）
   p.2〜: 業務一覧テーブル
   p.末〜: 優先順位 結果一覧テーブル
   ══════════════════════════════════════════════════════════ */
async function exportPDF() {
  ...
  try {
    ...
    // A4横: 297×210mm @ 150dpi → 1754×1240px 相当
    const PW_MM = 297, PH_MM = 210;
    const SCALE = 2; // 高解像度キャプチャ用倍率
    ...
    /* ─────────────────────────────────────────────────────
       p.1: チャート + 象限説明カード
       既存の #pane-chart と .q-legend-grid を組み合わせた
       専用コンテナを一時生成してキャプチャ
    ───────────────────────────────────────────────────── */
    const chartCanvas = document.getElementById('qc');
    const chartImgData = chartCanvas.toDataURL('image/png', 1.0);

    // カード HTML を取得（Web画面そのまま）
    const legendGrid = document.querySelector('.q-legend-grid');

    // 一時コンテナ生成
    const p1wrap = document.createElement('div');
    p1wrap.style.cssText = [
      'position:fixed','top:-9999px','left:0',
      'width:1120px',  // A4横の比率に近い幅
      'background:#ffffff',
    ...
    titleBar.style.cssText = 'background:#185FA5;color:#fff;padding:10px 18px;border-radius:8px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:center';
    titleBar.innerHTML = `<span style="font-size:17px;font-weight:600">AI活用業務 優先順位化レポート</span><span style="font-size:12px;opacity:.85">4象限マップ　${stamp}</span>`;
    p1wrap.appendChild(titleBar);

    // チャート画像
    const chartImg = document.createElement('img');
    chartImg.src = chartImgData;
    chartImg.style.cssText = 'width:100%;height:auto;display:block;margin-bottom:14px;border-radius:6px;border:0.5px solid #e0ded6';
    p1wrap.appendChild(chartImg);

    // 象限説明カード（Web の HTML をクローンして流用）
    const clonedGrid = legendGrid.cloneNode(true);
    ...
    // カードのスタイルを明示的にインライン化
    [...clonedGrid.querySelectorAll('.q-legend-card')].forEach(card => {
      card.style.cssText = (card.style.cssText || '') +
        ';background:#fff;border:0.5px solid #e0ded6;padding:10px 14px;border-radius:0 8px 8px 0;box-shadow:0 1px 3px rgba(0,0,0,.05)';
    });
    [...clonedGrid.querySelectorAll('.q-lbl')].forEach(el => {
      el.style.cssText = (el.style.cssText||'') + ';font-size:13px;font-weight:600;margin-bottom:4px';
    });
    [...clonedGrid.querySelectorAll('.q-desc')].forEach(el => {
      el.style.cssText = (el.style.cssText||'') + ';font-size:12px;color:#5f5e5a;line-height:1.5';
    });
    p1wrap.appendChild(clonedGrid);
    document.body.appendChild(p1wrap);

    const p1canvas = await html2canvas(p1wrap, { scale: SCALE, useCORS: true, backgroundColor:'#ffffff', logging:false });
    document.body.removeChild(p1wrap);

    await addCanvasPage(doc, p1canvas, PW_MM, PH_MM, false);
    addPageFooter(doc, stamp, pageNum++, PW_MM, PH_MM);

    /* ─────────────────────────────────────────────────────
       p.2〜: 業務一覧テーブル（html2canvas でキャプチャ）
    ───────────────────────────────────────────────────── */
    const bizPages = buildTableHTML('business', eligible, stamp);
    for (let i = 0; i < bizPages.length; i++) {
      doc.addPage([PW_MM, PH_MM], 'landscape');
      const pg = await captureHTML(bizPages[i], 1120);
      await addCanvasPage(doc, pg, PW_MM, PH_MM, false);
      addPageFooter(doc, stamp, pageNum++, PW_MM, PH_MM);
    }

    /* ─────────────────────────────────────────────────────
       p.末〜: 優先順位 結果一覧（html2canvas でキャプチャ）
    ───────────────────────────────────────────────────── */
    const resultPages = buildTableHTML('result', eligible, stamp);
    for (let i = 0; i < resultPages.length; i++) {
      doc.addPage([PW_MM, PH_MM], 'landscape');
      const pg = await captureHTML(resultPages[i], 1120);
      await addCanvasPage(doc, pg, PW_MM, PH_MM, false);
      addPageFooter(doc, stamp, pageNum++, PW_MM, PH_MM);
    }
...
function waitForLibs() {
  return new Promise(resolve => {
    const check = () => (window.jspdf && window.html2canvas) ? resolve() : setTimeout(check, 100);
    check();
  });
}

/* ── Canvas を PDF ページに貼り付け（余白付き・縦横比保持） ── */
async function addCanvasPage(doc, canvas, pwMM, phMM, addNewPage) {
  if (addNewPage) doc.addPage([pwMM, phMM], 'landscape');
  const margin = 8; // mm
  const maxW = pwMM - margin * 2;
  const maxH = phMM - margin * 2 - 8; // フッター分
...
/* ── フッター描画 ────────────────────────────────────────── */
function addPageFooter(doc, stamp, pageNum, PW, PH) {
  const M = 8;
  doc.setDrawColor(200,198,190); doc.setLineWidth(0.3);
  doc.line(M, PH - 10, PW - M, PH - 10);
  doc.setFont('helvetica','normal'); doc.setFontSize(7.5);
  doc.setTextColor(150,148,140);
  doc.text('AI活用業務 優先順位化ツール', M, PH - 5);
  doc.text(`${pageNum}  |  ${stamp}`, PW - M, PH - 5, { align:'right' });
}

/* ── HTML 文字列を一時 DOM に描画して html2canvas でキャプチャ ── */
async function captureHTML(htmlStr, width) {
  const wrap = document.createElement('div');
  wrap.style.cssText = `position:fixed;top:-9999px;left:0;width:${width}px;background:#ffffff`;
  wrap.innerHTML = htmlStr;
  document.body.appendChild(wrap);
  const c = await html2canvas(wrap, { scale:2, useCORS:true, backgroundColor:'#ffffff', logging:false });
  document.body.removeChild(wrap);
  return c;
}

/* ── テーブル HTML 生成（業務一覧 / 結果一覧） ──────────────
   1120px 幅のHTMLを生成し、rows_per_page行ごとに分割して返す。
   CSS はすべてインラインで記述し html2canvas で確実に描画する。
─────────────────────────────────────────────────────────── */
function buildTableHTML(type, eligible, stamp) {
  const ROWS_PER_PAGE = 12;
  const isBiz = type === 'business';

  const COMMON_STYLE = `
    font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Hiragino Sans","Meiryo",sans-serif;
    font-size:12px; color:#1a1a18; box-sizing:border-box;
  `;
  const TH_STYLE = `
    background:#f1efe8; padding:6px 8px; font-weight:600; font-size:11px;
    color:#5f5e5a; text-align:left; border-bottom:1px solid #c8c6be;
    border-right:0.5px solid #e0ded6; white-space:nowrap;
  `;
  const TD_STYLE = `
    padding:6px 8px; vertical-align:top; border-bottom:0.5px solid #dedad2;
    border-right:0.5px solid #ece9e0; line-height:1.45; font-size:11px;
  `;
  const TITLE = isBiz ? '業務一覧' : '優先順位 結果一覧';

  const Q_LABEL = { qw:'Quick Win', st:'Strategic', pt:'Potential', lp:'Low Priority' };
  const Q_COLOR = {
    qw:{ text:'#3B6D11', bg:'#EAF3DE' }, st:{ text:'#185FA5', bg:'#E6F1FB' },
    pt:{ text:'#854F0B', bg:'#FAEEDA' }, lp:{ text:'#5F5E5A', bg:'#F1EFE8' },
  };
  const FIT_LABEL = { high:'高適合', mid:'中適合', low:'低適合' };
  const FIT_COLOR = {
  ...
  const badge = (label, tc, bg) =>
    `<span style="display:inline-block;padding:2px 7px;border-radius:10px;font-size:10px;font-weight:600;color:${tc};background:${bg};white-space:nowrap">${label}</span>`;

  const mkRow = (b, ri) => {
    const si  = getLv('s', b.sRaw) * getLv('i', b.iRaw);
    const q   = getQ(getLv('p', b.pRaw), si);
    const qc  = Q_COLOR[q];  const fc = FIT_COLOR[b.fit];
    const bg  = ri % 2 === 0 ? '#ffffff' : '#f9f8f5';
    const fitBadge  = badge(FIT_LABEL[b.fit], fc.text, fc.bg);
    const qBadge    = badge(Q_LABEL[q],       qc.text, qc.bg);
    const pLv = getLv('p', b.pRaw), sLv = getLv('s', b.sRaw), iLv = getLv('i', b.iRaw);
    const td  = (content, extra='') =>
      `<td style="${TD_STYLE}background:${bg};${extra}">${content}</td>`;

    if (isBiz) {
      const q1 = (b.q1labels||[]).join('、') || '—';
      const desc = (b.desc||'—').replace(/</g,'&lt;');
      return `<tr>
        ${td(`<strong>${b.name.replace(/</g,'&lt;')}</strong>`)}
        ${td(b.dept||'—')}
        ${td(fitBadge,'text-align:center')}
        ${td(`Lv.${pLv}`,'text-align:center')}
        ${td(`Lv.${sLv}`,'text-align:center')}
        ${td(`Lv.${iLv}`,'text-align:center')}
        ${td(qBadge,'text-align:center')}
        ${td(q1)}
        ${td(desc,'border-left:2.5px solid '+qc.text+';')}
      </tr>`;
    } else {
      const hint = (aiHint(b, q)||'').replace(/</g,'&lt;');
      return `<tr>
        ${td(`<strong>${b.name.replace(/</g,'&lt;')}</strong>`)}
        ${td(b.dept||'—')}
        ${td(fitBadge,'text-align:center')}
        ...
        ${td(`Lv.${iLv}`,'text-align:center')}
        ${td(qBadge,'text-align:center')}
        ${td(hint,'border-left:2.5px solid '+qc.text+';')}
      </tr>`;
    }
  };
  ...
  const mkHeader = () => {
    const ths = isBiz
      ? ['業務名','部門','適合性','P','S','I','分類','Q1 作業種別','主な作業内容']
      : ['業務名','部門','適合性','P','S','I','分類','AI活用の可能性'];
    const ws  = isBiz
      ? ['14%','9%','7%','4%','4%','4%','9%','16%','33%']
      : ['13%','9%','7%','4%','4%','4%','9%','50%'];
    return ths.map((h,i)=>`<th style="${TH_STYLE}width:${ws[i]};text-align:${['適合性','P','S','I','分類'].includes(h)?'center':'left'}">${h}</th>`).join('');
  };

  const wrapPage = (rows, pageIdx, totalPages) => `
    <div style="${COMMON_STYLE}padding:28px 32px;background:#fff;width:1120px;min-height:740px">
      <div style="background:#185FA5;color:#fff;padding:9px 16px;border-radius:7px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:15px;font-weight:600">AI活用業務 優先順位化レポート</span>
        <span style="font-size:12px;opacity:.85">${TITLE}${totalPages>1?' ('+(pageIdx+1)+'/'+totalPages+')':''}&ensp;${stamp}</span>
      </div>
      <table style="width:100%;border-collapse:collapse;table-layout:fixed">
        <thead><tr>${mkHeader()}</tr></thead>
        <tbody>${rows.map((b,ri)=>mkRow(b,ri)).join('')}</tbody>
      </table>
    </div>`;

  const pages = [];
  for (let i = 0; i < eligible.length; i += ROWS_PER_PAGE) {
    pages.push(wrapPage(eligible.slice(i, i + ROWS_PER_PAGE), pages.length, Math.ceil(eligible.length / ROWS_PER_PAGE)));
  }
  return pages;
}
